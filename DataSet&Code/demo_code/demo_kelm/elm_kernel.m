function [TrainingTime, TestingTime, TrainingAccuracy, TestingAccuracy,elm_pred] = elm_kernel(train_data, test_data, Elm_Type, Regularization_coefficient, Kernel_type, Kernel_para)
...
if strcmp(kernel_type,'RBF_kernel'),
    if nargin<4,
        ...
        sum_xtrain = zeros(size(Xtrain,1),1);
        for i = 1:size(Xtrain,1)
            sum_xtrain(i) = Xtrain(i,:)*Xtrain(i,:)';
        end
        XXh1 = sum_xtrain*ones(1,size(Xt,1));
%         XXh1 = sum(bsxfun(@power,Xtrain,2),2)*ones(1,size(Xt,1));
